 ...
 * signal must be scaled down by log2(numTaps) bits (where log2 is read as log
 * to the base 2).
 *
 * \par
 * Refer to the function <code>arm_fir_decimate_q31()</code> for a slower
 ...
 * structure. Use the function <code>arm_fir_decimate_init_q31()</code> to

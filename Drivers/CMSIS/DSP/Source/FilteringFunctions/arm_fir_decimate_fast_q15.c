 ...
 *overflows it wraps around and distorts the result. In order to avoid overflows
 *completely the input signal must be scaled down by log2(numTaps) bits (log2 is
 *read as log to the base 2). The 2.30 accumulator is then truncated to 2.15
 *format and saturated to yield the 1.15 result.
 *
 * \par
 * Refer to the function <code>arm_fir_decimate_q15()</code> for a slower
 ...
 *structure. Use the function <code>arm_fir_decimate_init_q15()</code> to

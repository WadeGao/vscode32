 ...
 * Input samples need to be downscaled by 1 bit to avoid saturations in the Q31
 * DCT process, as the conversion from DCT2 to DCT4 involves one subtraction.
 * Internally inputs are downscaled in the RFFT process function to avoid
 * overflows. Number of bits downscaled, depends on the size of the transform.
 * The input and output formats for different DCT sizes and number of bits to
 * upscale are mentioned in the table below:
 *

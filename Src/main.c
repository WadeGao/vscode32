...
int main(void)
{
  ...
  // const uint64_t kTim3AutoLoadVal = __HAL_TIM_GetAutoreload(&htim3);
  // __HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_1, kTim3AutoLoadVal / 100);

  const volatile uint64_t kTim14AutoLoadVal = __HAL_TIM_GetAutoreload(&htim14);
  __HAL_TIM_SetCompare(&htim14, TIM_CHANNEL_1, kTim14AutoLoadVal);
  ...
  while (1) {
    uint16_t pwmVal = 0;
    while (pwmVal < __HAL_TIM_GET_AUTORELOAD(&htim14)) {
      pwmVal++;

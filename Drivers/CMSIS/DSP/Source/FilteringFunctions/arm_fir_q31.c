 ...
 * by log2(numTaps) bits. After all multiply-accumulates are performed, the 2.62
 * accumulator is right shifted by 31 bits and saturated to 1.31 format to yield
 * the final result.
 *
 * \par
 * Refer to the function <code>arm_fir_fast_q31()</code> for a faster but less
...
void arm_fir_q31(const arm_fir_instance_q31 *S, q31_t *pSrc, q31_t *pDst,
  ...
  while (blkCnt > 0U) {
    ...
    *pStateCurnt++ = *pSrc++;
    ...
    /* If the filter length is not a multiple of 3, compute the remaining filter
     * taps */
  ...
  /* If the blockSize is not a multiple of 3, compute any remaining output
   *samples here.

 ...
 * ************************************************************************
 */
void OLED_ShowStr(signed short int x, signed short int y, unsigned char ch[],
    ...
    switch (TextSize) {
      case 1: {
        while (ch[j] != '\0') {
          ...
          if (x >= 125 ||
              (127 - x < 6))  // 一行最大显示字符数：21字节显示,多出两列，不显示
                              // || 剩余列小于6不能显示完整字符，换行显示
          {
            x = 0;
            y += 8;          // 换行显示
          ...
          for (unsigned char m = 0; m < 2; m++) {
            for (unsigned char n = 0; n < 8; n++) {
              for (unsigned char i = 0; i < 8; i++) {
                OLED_SetPixel(x + n, y + i + m * 8,
                              (F8X16[c][n + m * 8] >> i) & 0x01);
              }
            }
          }
          x += 8;
 ...
 * ************************************************************************
 */
void OLED_ShowChinese(signed short int x, signed short int y,
                      unsigned char *ch) {
  if (x >= 0 && x < SCREEN_COLUMN && y >= 0 && y < SCREEN_ROW) {
    ...
    while (ch[len] != '\0') {
      if (x >= 127 ||
          (127 - x <
           16))  // 8个汉字显示||剩余列小于16不能显示完整字符，换行显示
      {
        x = 0;
        y += 16;
        if (63 - y < 16)  // 不足以显示一行时不显示
      ...
      // 需要处理输入数据大于显示数据的问题
      for (unsigned char i = 0; i < sizeof(F16x16_CN) / sizeof(GB2312_CN);
           i++) {
        if (((F16x16_CN[i].index[0] == ch[len]) &&
          ...
          for (unsigned char m = 0; m < 2; m++)  // 页
          {
            for (unsigned char n = 0; n < 16; n++)  // 列
            {
              ...
              {
                OLED_SetPixel(x + n, y + j + m * 8,
                              (F16x16_CN[i].encoder[n + m * 16] >> j) & 0x01);
              }
            }
          }
          x += 16;
 ...
 * ************************************************************************
 */
void OLED_ShowBMP(signed short int x0, signed short int y0, signed short int L,
                  signed short int H, const unsigned char BMP[]) {
  if (x0 >= 0 && x0 < SCREEN_COLUMN && x0 + L <= SCREEN_ROW && y0 >= 0 &&
      y0 < SCREEN_COLUMN && y0 + H <= SCREEN_ROW) {

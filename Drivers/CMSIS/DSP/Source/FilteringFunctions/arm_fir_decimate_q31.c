 ...
 * by log2(numTaps) bits (where log2 is read as log to the base 2). After all
 * multiply-accumulates are performed, the 2.62 accumulator is truncated to 1.32
 * format and then saturated to 1.31 format.
 *
 * \par
 * Refer to the function <code>arm_fir_decimate_fast_q31()</code> for a faster
